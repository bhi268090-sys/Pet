...
from cubepet_config import (
    DEFAULT_ACTIVE_GRACE_S,
    DEFAULT_EDITOR_HEIST_MAX_S,
    DEFAULT_EDITOR_HEIST_MIN_S,
    DEFAULT_EDITOR_HEIST_RETRY_MAX_S,
    DEFAULT_EDITOR_HEIST_RETRY_MIN_S,
    DEFAULT_HUNGER_FULL_S,
    DEFAULT_IMAGE_IDLE_S,
    DEFAULT_IMAGE_MAX_S,
    DEFAULT_IMAGE_MIN_S,
    DEFAULT_PROFILE_ID,
    DEFAULT_WINDOW_KILL_MAX_S,
    DEFAULT_WINDOW_KILL_MIN_S,
    DEFAULT_WINDOW_KILL_RETRY_MAX_S,
    DEFAULT_WINDOW_KILL_RETRY_MIN_S,
    DEFAULT_WINDOW_KILL_SUCCESS_MAX_S,
    DEFAULT_WINDOW_KILL_SUCCESS_MIN_S,
    DEFAULT_WINDOW_KILL_TRIGGER_CHANCE,
    DISCORD_RPC_DEFAULT_CLIENT_ID,
    ENV_ACTIVE_GRACE_S,
    ENV_DEBUG,
    ENV_DISCORD_RPC,
    ENV_DISCORD_RPC_CLIENT_ID,
    ENV_EDITOR_MISCHIEF,
    ENV_FALSE_VALUES,
    ENV_HORROR_GAME,
    ENV_HUNGER,
    ...
    IMAGE_EXTS_WITH_PIL,
    MIN_HUNGER_FULL_S,
    MIN_IMAGE_HEIST_START_S,
    SETTINGS_FILE_VERSION,
    TOKEN_RE,
)
from pet_profiles import PET_PROFILES, PetProfile

from ui_credits import show_credits_popup
try:
...
class AnnoyingBlockPet:
    def __init__(self) -> None:
        ...
        # - CUBEPET_IMAGE_IDLE_S=2.0 minimum idle seconds for image-heist (default 2.0)
        ...
        # - CUBEPET_HUNGER=0 disables the hunger bar + feeding (default on)
        # - CUBEPET_HUNGER_FULL_S=900 seconds to drain from full->empty (default 900)
        # - CUBEPET_EDITOR_MISCHIEF=0 disables prank-editor typing interference (default on)
        if getattr(sys, 'frozen', False):
            self.asset_dir = Path(sys._MEIPASS)
        else:
        ...
        try:
            icon_path = self.asset_dir / "Frech_Birld.ico"
            if icon_path.exists():
        ...
        self._fg_cache_until = 0.0
        self._fg_cache_hwnd = 0
        self._fg_cache_title = ""
        self._fg_cache_proc = ""
        ...
        # Privacy/perf: only scan the filesystem for images when the user enables Image-Heist.
        self.image_paths: list[Path] = []
        self._image_scan_in_progress = False
        if bool(getattr(self, "image_heist_enabled", False)):
        ...
        # Schedule disruptive cursor/window pranks only when explicitly enabled.
        if bool(getattr(self, "window_kill_enabled", False)):
            self.next_window_kill_at = now + random.uniform(
                DEFAULT_WINDOW_KILL_MIN_S, DEFAULT_WINDOW_KILL_MAX_S
            )
        else:
            self.next_window_kill_at = now + 10**9

        if bool(getattr(self, "mouse_lock_enabled", False)):
            self.next_mouse_lock_at = now + random.uniform(45.0, 120.0)
        else:
            self.next_mouse_lock_at = now + 10**9
        self.close_prompt_window: tk.Toplevel | None = None
        self.youtube_prompt_window: tk.Toplevel | None = None
        self.credits_window: tk.Toplevel | None = None
        ...
        now = time.monotonic()
        # Schedule optional "pranks" only when enabled.
        if bool(getattr(self, "image_heist_enabled", False)):
            self.next_image_heist_at = now + random.uniform(self.image_min_s, self.image_max_s)
        else:
            self.next_image_heist_at = now + 10**9

        if bool(getattr(self, "editor_heist_enabled", False)):
            self.next_editor_heist_at = now + random.uniform(
                DEFAULT_EDITOR_HEIST_MIN_S, DEFAULT_EDITOR_HEIST_MAX_S
            )
        else:
            self.next_editor_heist_at = now + 10**9

        if bool(getattr(self, "clones_enabled", True)):
            self.next_clone_spawn_at = now + random.uniform(60.0, 140.0)
        else:
            self.next_clone_spawn_at = now + 10**9
        ...
        if bool(getattr(self, "show_options_on_start", False)):
            self.root.after(50, self._show_options_window)
    ...
    def _save_persistent_settings(self) -> None:
        try:
            data = {
                ...
                "hunger_enabled": bool(getattr(self, "hunger_enabled", True)),
                "editor_mischief_enabled": bool(getattr(self, "editor_mischief_enabled", True)),
                # Prank feature toggles (enabled-by-default; persisted so the UI matches behavior).
                "image_heist_enabled": bool(getattr(self, "image_heist_enabled", True)),
                "editor_heist_enabled": bool(getattr(self, "editor_heist_enabled", True)),
                "clones_enabled": bool(getattr(self, "clones_enabled", True)),
                "cursor_games_enabled": bool(getattr(self, "cursor_games_enabled", True)),
                "mouse_lock_enabled": bool(getattr(self, "mouse_lock_enabled", True)),
                "window_kill_enabled": bool(getattr(self, "window_kill_enabled", True)),
    ...
    def _load_settings(self) -> None:
        ...
        self.hunger_enabled = bool(persisted.get("hunger_enabled", True))
        self.editor_mischief_enabled = bool(persisted.get("editor_mischief_enabled", True))

        # Prank feature toggles. Enabled by default; disable via Options or environment variables.
        self.image_heist_enabled = bool(persisted.get("image_heist_enabled", True))
        self.editor_heist_enabled = bool(persisted.get("editor_heist_enabled", True))
        self.clones_enabled = bool(persisted.get("clones_enabled", True))
        self.cursor_games_enabled = bool(persisted.get("cursor_games_enabled", True))
        self.mouse_lock_enabled = bool(persisted.get("mouse_lock_enabled", True))
        self.window_kill_enabled = bool(persisted.get("window_kill_enabled", True))
        try:
        ...
        self.image_heist_enabled = self._apply_optional_env_bool(
            ENV_IMAGE_HEIST, self.image_heist_enabled
        )
        self.editor_heist_enabled = self._apply_optional_env_bool(
            ENV_EDITOR_HEIST, self.editor_heist_enabled
        )
        self.clones_enabled = self._apply_optional_env_bool(ENV_CLONES, self.clones_enabled)
        self.cursor_games_enabled = self._apply_optional_env_bool(
            ENV_CURSOR_GAMES, self.cursor_games_enabled
        )
        self.mouse_lock_enabled = self._apply_optional_env_bool(
            ENV_MOUSE_LOCK, self.mouse_lock_enabled
        )
        self.window_kill_enabled = self._apply_optional_env_bool(
            ENV_WINDOW_KILL, self.window_kill_enabled
        )

        # Cursor-manipulating pranks require cursor games to be enabled.
        if self.mouse_lock_enabled or self.window_kill_enabled:
            self.cursor_games_enabled = True
    ...
    def _load_image_cache(self, max_files: int) -> list[Path]:
        ...
        try:
            ...
            out: list[Path] = []
            exts = IMAGE_EXTS_WITH_PIL if PIL_AVAILABLE else IMAGE_EXTS_TK_ONLY
    ...
    def _save_image_cache(self, paths: list[Path]) -> None:
        try:
            ...
            tmp = self.image_cache_path.with_suffix(self.image_cache_path.suffix + ".tmp")
            tmp.write_text(txt, encoding="utf-8")
            tmp.replace(self.image_cache_path)
        except Exception:
            pass
    ...
    def _start_image_scan_background(self, max_files: int) -> None:
        ...
        def worker() -> None:
            try:
                ...
                # Keep this worker independent from Tk calls to avoid cross-thread Tcl errors.
    ...
    def _update_discord_rpc(self, force: bool = False) -> None:
        ...
        try:
            self._discord_rpc.update(
                details=f"System Audit Active: {getattr(self.pet_profile, 'name', 'CubePet')}",
                state="Performing Security Scan...", # Professional status
                large_image="shield_icon", # Assuming we don't have this key, but standard rpc might ignore or show default
                large_text="IT Security Module",
                start=self._hg_started_at if self._hg_active else time.time(), # Show elapsed time
            )
            self._last_rpc_update_t = now
        except Exception:
    ...
    def _on_escape(self, _event=None):
        ...
        try:
            ...
            if getattr(self, "dying", False) and getattr(self, "final_window", None) is not None:
                self._resurrect_pet()
                return "break"
            # Force exit to prevent double-open / zombie processes
            os._exit(0)
        except Exception:
            os._exit(0)
        return "break"
    ...
    def _start_drag(self, event) -> None:
        if (
            ...
            or self.youtube_prompt_window is not None
            or self.credits_window is not None
        ...
        if was_click:
            ...
            if dist >= 120.0 and not self.intro_active:
                ...
                if self.drag_shoo_count >= 2:
                    self.drag_shoo_count = 0
                    self.drag_shoo_until = 0.0
                    if bool(getattr(self, "cursor_games_enabled", False)):
    ...
    def _motion_loop(self) -> None:
        ...
        if self.heist_active:
            ...
            if now >= self.stunned_until:
                cursor_target = None
                if (
                    bool(getattr(self, "cursor_games_enabled", False))
    ...
    def _start_window_kill(self) -> bool:
        ...
        # If the foreground app is a browser, prefer closing a tab (Ctrl+W) rather than
        # clicking the window's X (which closes the whole browser).
        try:
            proc = (self._get_window_process_name(int(hwnd)) or "").lower()
        except Exception:
            proc = ""
        browsers = {
            "chrome.exe",
        ...
        if proc in browsers:
            try:
                ...
                self._ding()
                self.angry_until = time.monotonic() + 1.1
                return True
            except Exception:
                # Fall back to cursor-click close below.
                pass
    ...
    def _show_close_prompt(self) -> None:
        ...
        opt_btn.pack(side="left", padx=(0, 6))
        credits_btn = tk.Button(
            buttons,
            text="Credits",
            width=9,
            command=self._close_prompt_credits,
            bg="#ffffff",
            fg="#111111",
            relief="flat",
        )
    ...
    def _close_prompt_credits(self) -> None:
        self._hide_close_prompt()
        self._show_credits_popup()

    def _show_credits_popup(self) -> None:
        self._hide_credits_popup()
        self._hide_options_window()

        px = int(self.x + self.block_size + 10)
        py = int(self.y - 10)
        self.credits_window = show_credits_popup(
            self.root,
            anchor_x=px,
            anchor_y=py,
    ...
    def _hide_credits_popup(self) -> None:
    ...
    def _hide_options_window(self) -> None:
        ...
        self._opt_image_heist_var = None
        self._opt_editor_heist_var = None
        self._opt_clones_var = None
        self._opt_cursor_games_var = None
        self._opt_mouse_lock_var = None
        self._opt_window_kill_var = None
    ...
    def _show_options_window(self) -> None:
        ...
        self._opt_image_heist_var = tk.BooleanVar(
            value=bool(getattr(self, "image_heist_enabled", False))
        )
        self._opt_editor_heist_var = tk.BooleanVar(
            value=bool(getattr(self, "editor_heist_enabled", False))
        )
        self._opt_clones_var = tk.BooleanVar(value=bool(getattr(self, "clones_enabled", True)))
        self._opt_cursor_games_var = tk.BooleanVar(
            value=bool(getattr(self, "cursor_games_enabled", False))
        )
        self._opt_mouse_lock_var = tk.BooleanVar(
            value=bool(getattr(self, "mouse_lock_enabled", False))
        )
        self._opt_window_kill_var = tk.BooleanVar(
            value=bool(getattr(self, "window_kill_enabled", False))
        )
        ...
        def _on_toggle() -> None:
            try:
                ...
                if self._opt_image_heist_var is not None:
                    self.image_heist_enabled = bool(self._opt_image_heist_var.get())
                if self._opt_editor_heist_var is not None:
                    self.editor_heist_enabled = bool(self._opt_editor_heist_var.get())
                if self._opt_clones_var is not None:
                    self.clones_enabled = bool(self._opt_clones_var.get())
                if self._opt_cursor_games_var is not None:
                    self.cursor_games_enabled = bool(self._opt_cursor_games_var.get())
                if self._opt_mouse_lock_var is not None:
                    self.mouse_lock_enabled = bool(self._opt_mouse_lock_var.get())
                if self._opt_window_kill_var is not None:
                    self.window_kill_enabled = bool(self._opt_window_kill_var.get())
                if self._opt_start_var is not None:
                    self.show_options_on_start = bool(self._opt_start_var.get())

                # Dependencies: mouse-lock/window-kill require cursor games (cursor control).
                if self.mouse_lock_enabled or self.window_kill_enabled:
                    self.cursor_games_enabled = True
                    if self._opt_cursor_games_var is not None:
                        self._opt_cursor_games_var.set(True)

                # If a feature is turned off while active, stop it immediately.
                if not self.cursor_games_enabled:
                    self.cursor_pingpong_active = False
                    self.cursor_heist_active = False
                    self.mouse_lock_active = False
                    self.window_kill_active = False
                    self.close_attack_active = False
                    self.angry_until = 0.0
                if not self.clones_enabled:
                    self._destroy_clone()
                    self.next_clone_spawn_at = now + 10**9
                if not self.image_heist_enabled:
                    if self.heist_active and self.heist_kind == "image":
                        self._stop_heist()
                    self.next_image_heist_at = now + 10**9
                else:
                ...
                if not self.editor_heist_enabled:
                    if self.heist_active and self.heist_kind == "editor":
                        self._stop_heist()
                    self.next_editor_heist_at = now + 10**9
                else:
                    self.next_editor_heist_at = now + random.uniform(
                ...
                if self.clones_enabled and self.clone_window is None and now >= self.next_clone_spawn_at:
                    self.next_clone_spawn_at = now + random.uniform(60.0, 140.0)
                ...
                if not self.window_kill_enabled:
                    self.window_kill_active = False
                    self.next_window_kill_at = now + 10**9
                else:
                    self.next_window_kill_at = now + random.uniform(
        ...
        pranks1 = tk.Frame(frame, bg="#dbeafe")
        pranks1.pack(fill="x", padx=10, pady=(0, 6))
        tk.Checkbutton(
            pranks1,
            text="Image-Heist",
            bg="#dbeafe",
            ...
            activebackground="#dbeafe",
            activeforeground="#0f172a",
            variable=self._opt_image_heist_var,
            command=_on_toggle,
        ).pack(side="left")
        tk.Checkbutton(
            pranks1,
            ...
            activebackground="#dbeafe",
            activeforeground="#0f172a",
            variable=self._opt_editor_heist_var,
            command=_on_toggle,
        ).pack(side="left", padx=(10, 0))
        tk.Checkbutton(
            pranks1,
            ...
            activebackground="#dbeafe",
            activeforeground="#0f172a",
            variable=self._opt_clones_var,
            command=_on_toggle,
        ).pack(side="left", padx=(10, 0))

        pranks2 = tk.Frame(frame, bg="#dbeafe")
        pranks2.pack(fill="x", padx=10, pady=(0, 6))
        tk.Checkbutton(
            pranks2,
            text="Cursor-Games",
            bg="#dbeafe",
            ...
            activebackground="#dbeafe",
            activeforeground="#0f172a",
            variable=self._opt_cursor_games_var,
            command=_on_toggle,
        ).pack(side="left")
        tk.Checkbutton(
            pranks2,
            ...
            activebackground="#dbeafe",
            activeforeground="#0f172a",
            variable=self._opt_mouse_lock_var,
            command=_on_toggle,
        ).pack(side="left", padx=(10, 0))
        tk.Checkbutton(
            pranks2,
            ...
            activebackground="#dbeafe",
            activeforeground="#0f172a",
            variable=self._opt_window_kill_var,
            command=_on_toggle,
        ).pack(side="left", padx=(10, 0))
    ...
    def _annoy_loop(self) -> None:
        ...
        if not self._dragging and not self.intro_active:
            if random.random() < 0.35:
                self.root.lift()
            if self.allow_focus_steal and (not user_active) and random.random() < 0.18:
                try:
            ...
            if self._can_start_major_prank(now, user_active=user_active):
                if bool(getattr(self, "editor_heist_enabled", False)) and now >= self.next_editor_heist_at:
                        if editor_idle_ok and self._start_editor_heist():
                            ...
                            self.next_editor_heist_at = now + random.uniform(
                                DEFAULT_EDITOR_HEIST_RETRY_MIN_S, DEFAULT_EDITOR_HEIST_RETRY_MAX_S
                            )
                elif bool(getattr(self, "image_heist_enabled", False)) and now >= self.next_image_heist_at:
                    if image_idle_ok and self._start_image_heist():
                        ...
                        self.next_image_heist_at = now + random.uniform(12.0, 25.0)
        ...
        if (
            ...
            and now >= self.angry_until
            and bool(getattr(self, "window_kill_enabled", False))
            and bool(getattr(self, "cursor_games_enabled", False))
        ):
            if random.random() < DEFAULT_WINDOW_KILL_TRIGGER_CHANCE:
                if self._start_window_kill():
                    self.next_window_kill_at = now + random.uniform(
                        DEFAULT_WINDOW_KILL_SUCCESS_MIN_S, DEFAULT_WINDOW_KILL_SUCCESS_MAX_S
                    )
                else:
                    self.next_window_kill_at = now + random.uniform(
                        DEFAULT_WINDOW_KILL_RETRY_MIN_S, DEFAULT_WINDOW_KILL_RETRY_MAX_S
                    )
        ...
        if (
            ...
            and (not user_active)
            and now >= self.angry_until
            and bool(getattr(self, "mouse_lock_enabled", False))
            and bool(getattr(self, "cursor_games_enabled", False))
        ):
            ...
            if random.random() < 0.15:
                ...
                self.next_mouse_lock_at = now + random.uniform(60.0, 210.0)
                self.confused_until = max(self.confused_until, self.mouse_lock_until)
            else:
                self.next_mouse_lock_at = now + random.uniform(30.0, 60.0)

        if (
            not self.intro_active
            and self.clone_window is None
            and now >= self.next_clone_spawn_at
            and bool(getattr(self, "clones_enabled", True))
    ...
    def _foreground_is_browser_youtube(self) -> bool:
        hwnd, title, proc = self._get_foreground_snapshot()
        if not hwnd:
            return False
        title = title.lower()
        if not title:
    ...
    def _foreground_is_discord(self) -> bool:
        hwnd, _, proc = self._get_foreground_snapshot()
        if not hwnd:
            return False
        proc = proc.lower()
        if not proc:
    ...
    def _get_foreground_snapshot(self, cache_ttl_s: float = 0.2) -> tuple[int, str, str]:
        if USER32 is None:
            return (0, "", "")
        now = time.monotonic()
        ttl = max(0.05, min(cache_ttl_s, 1.0))
        if now <= self._fg_cache_until:
            return (self._fg_cache_hwnd, self._fg_cache_title, self._fg_cache_proc)

        hwnd = int(USER32.GetForegroundWindow() or 0)
        title = self._get_window_title(hwnd) if hwnd else ""
        proc = self._get_window_process_name(hwnd) if hwnd else ""
        self._fg_cache_hwnd = hwnd
        self._fg_cache_title = title
        self._fg_cache_proc = proc
        self._fg_cache_until = now + ttl
    ...
    def _on_discord_foreground(self) -> None:
        # Privacy: do not scrape other apps' config files. Only use user-provided/cached value.
    ...
    def _start_image_heist(self) -> bool:
        if self.heist_payload_window is not None:
            return False
        if not self.image_paths:
            if getattr(self, "_image_scan_in_progress", False):
                return False
    ...
    def _spawn_clone(self, now: float) -> None:
        ...
        self.next_clone_spawn_at = now + random.uniform(60.0, 160.0)
        self._ding()
